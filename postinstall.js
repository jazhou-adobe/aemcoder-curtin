...
[
  ...
  const destPath = path.resolve(__dirname, 'scripts', file.to);
  fs.mkdirSync(path.dirname(destPath), { recursive: true });
  fs.copyFileSync(path.resolve(__dirname, 'node_modules', file.from), destPath);
});
...
function checkSourceMaps() {
  ...
  if (!fs.existsSync(hlxIgnorePath) || !fs.readFileSync(hlxIgnorePath, 'utf-8').includes('*.map')) {
    console.info('Warning: Sourcemaps may be added to the repo. Please remove the *.map files or add "*.map" to .hlxignore before going live!\n');
  }
}
